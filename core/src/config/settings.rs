...
impl Settings {
    /// Parse settings from a TOML string. Useful for native `from_file` and wasm usage.
    pub fn from_toml_str(toml_str: &str) -> Result<Self, toml::de::Error> {
        toml::from_str::<Settings>(toml_str)
    }
    ...
    pub fn from_toml_or_json(toml_str: &str) -> Result<Self, CoreError> {
        // Try TOML first, fallback to JSON
        match toml::from_str::<Settings>(toml_str) {
            Ok(s) => Ok(s),
            Err(_) => serde_json::from_str::<Settings>(toml_str).map_err(|e| CoreError::Config(e.to_string())),
        }
    }

    pub fn save_to_file(&self, path: &str) -> Result<(), CoreError> {
        let toml_string = toml::to_string(self).map_err(|e| CoreError::Serialization(e.to_string()))?;
        std::fs::write(path, toml_string).map_err(|e| CoreError::Storage(e.to_string()))?;
        Ok(())
    }
    ...
    pub fn validate(&self) -> Result<(), CoreError> {
        if self.tp_percent <= 0.0 {
            return Err(CoreError::Config("tp_percent must be > 0".to_string()));
        }
        if self.sl_percent >= 0.0 {
            return Err(CoreError::Config("sl_percent must be < 0".to_string()));
        }
        if self.buy_amount <= 0.0 {
            return Err(CoreError::Config("buy_amount must be > 0".to_string()));
        }
        if self.timeout_secs <= 0 {
            return Err(CoreError::Config("timeout_secs must be > 0".to_string()));
        }
        if self.cache_capacity == 0 {
            return Err(CoreError::Config("cache_capacity must be > 0".to_string()));
        }
        if self.max_holded_coins == 0 {
            return Err(CoreError::Config("max_holded_coins must be > 0".to_string()));
        }
